...
- ivAES       (24 bytes)
- ivSerpent   (24 bytes)
- encrypted header  (32 bytes internal_key1 + 32 internal_key2 + 32 bytes internal_key3 + original fileName + 16 bytes Mac * 3)
- encrypted file (original fileSize + 16 bytes Mac * 3)

Notes:<br>
ivChaCha20, ivAES and ivSerpent, the 3 nonces (or initialization vector) used for encrypt the header are incremented for encrypt the file to ensure they are never reused with the same key.<br>

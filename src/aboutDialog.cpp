    ...
    : QDialog(parent), m_ui(new Ui::AboutDialog)
{
    ...
    if (QFile file("://about_files/gpl-3.0.html"); file.open(QFile::ReadOnly | QFile::Text))
        m_ui->gpllicenseEdit->setHtml(QTextStream(&file).readAll());

    if (QFile file2("://about_files/botan_license.txt"); file2.open(QFile::ReadOnly | QFile::Text))
        m_ui->botanlicenseEdit->setText(QTextStream(&file2).readAll());

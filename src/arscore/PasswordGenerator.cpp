...
QString PasswordGenerator::generatePassword() const
{
    ...
    if (m_flags & CharFromEveryGroup) {
        ...
        // shuffle chars (Fisher-Yates)
        for (int i = password.size() - 1; i >= 1; --i) {
            const int j = randomGen()->randomUInt(static_cast<quint32>(i + 1));
...
QVector<PasswordGroup> PasswordGenerator::passwordGroups() const
{
    ...
    // Remove excluded characters from each group, then drop empty groups
    for (auto& group : passwordGroups)
        group.removeIf([&](QChar ch) { return m_excluded.contains(ch); });
    passwordGroups.removeIf([](const PasswordGroup& g) { return g.isEmpty(); });

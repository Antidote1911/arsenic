...
QString errorCodeToString(quint32 error_code)
{
    ...
    switch (error_code) {
        ...
        case SRC_CHUNK_READ_ERROR:
            ret_string += QObject::tr("Can't read a data chunk (file may be truncated or corrupted).");
            break;
        ...
        case GLOBAL_MAC_FAIL:
            ret_string += QObject::tr("Global MAC verification failed. File may be truncated, appended or corrupted.");
            break;

        case CHANGE_PASSWORD_SUCCESS:
            ret_string += QObject::tr("Password changed successfully.");
            break;

        case WRONG_OLD_PASSWORD:
            ret_string += QObject::tr("Wrong old password or corrupted file header.");
            break;
    }
    return (ret_string);
}

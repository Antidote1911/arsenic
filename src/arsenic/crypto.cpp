...
void Crypto_Thread::setParam(bool direction, QStringList filenames, const QString password, const QString algo,int argonmem,int argoniter, bool deletefile)
{
...
void Crypto_Thread::run()
{
    for (auto& inputFileName : m_filenames)
    {
        ...
        else
        {
            ...
            {
                emit statusMessage("Error: Can't decrypt file. Wrong password entered or the file has been corrupted.");
            ...
            {
                emit statusMessage("Error: Can't decrypt file. Is it an encrypted file ?");
                QFile::remove(outfileresult);
...
void Crypto_Thread::encrypt(const QString src_path)
{
    size_t memlimit=0;
        ...
        std::unique_ptr<Botan::AEAD_Mode> enc = Botan::AEAD_Mode::create(m_algo.toStdString(), Botan::ENCRYPTION);
...
Botan::SecureVector<char> Crypto_Thread::convertStringToSecureVector(QString password)
{
    Botan::SecureVector<char> pass_buffer(CIPHER_KEY_LEN);
...
Botan::SecureVector<quint8> Crypto_Thread::calculateHash(Botan::SecureVector<char> pass_buffer,

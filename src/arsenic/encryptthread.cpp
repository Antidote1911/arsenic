...
EncryptThread::EncryptThread(FileSystemModel *arg_model, const QString &arg_password, const QString &arg_userName,QString ext,QString cryptoAlgo,int argonMemory,int argonOps,bool
...
std::vector<int> EncryptThread::getStatus() const
{
	if(this->isRunning())
...
const std::vector<FileInfoPtr> &EncryptThread::getItemList() const
{
	if(this->isRunning())
		return empty_list;
	else
...
void EncryptThread::interruptionPoint()
{
...
void EncryptThread::runHelper()
{
	...
	for(int i = 0; i < total_items; i++)
	{
		...
		// make sure the item exists
                ...
                if(item_type != FileInfo::typeToString(FileInfoPublic::MFIT_EMPTY))
		{
			QuaZip qz(zip_path);
                        ...
                        if(item_type == FileInfo::typeToString(FileInfoPublic::MFIT_FILE))
			{
				...
				if(ret_val)
				{
                    ...
                    qz.setComment(FileInfo::typeToString(FileInfoPublic::MFIT_FILE));
					qz.close();
				}
			}
			else
			{
                ...
                qInfo() << "folder size is: " + formatSize(size);
                    ...
                    {
                        qz.open(QuaZip::mdAdd);
                        qz.setComment(FileInfo::typeToString(FileInfoPublic::MFIT_DIR));
                        qz.close();
                    }
...
int EncryptThread::getNextFilenameSuffix(const QString &path, const QString &base_name, const
...
int EncryptThread::myEncryptFile(const QString &des_path, const QString &src_path, const QString &key,const QString &userName ,int argonmem,int iter, QString cryptoAlgo)
{
    size_t memlimit=0;
    size_t iterations=0;

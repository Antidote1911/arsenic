...
FileInfo::FileInfo(FileSystemModel *arg_model) : QObject(arg_model), type(MFIT_EMPTY),
...
FileInfo::FileInfo(FileSystemModel *arg_model, const QString &arg_path) :
        FileInfo(arg_model)
{
...
void FileInfo::setItem(const QString &arg_path)
{
...
void FileInfo::setItemHelper(const QString &arg_path)
{
	...
	if(q_file.isDir())
	{
		...
		size = CALCULATING_SIZE;
                ...
                dir_thread = new DirThread(q_file.absoluteFilePath());
                connect(dir_thread, &DirThread::updateSize, this, &FileInfo::updateSize);
...
QString FileInfo::getType() const
{
...
QString FileInfo::getSuffix() const
{
	if(type == MFIT_FILE)
	{
...
QString FileInfo::typeToString(FileInfoType arg_type)
{
	switch(arg_type)
	{
		case MFIT_DIR:
...
void FileInfo::updateSize(qint64 arg_size)
{
	size = arg_size;

	// tell the model to signal the view that its size data changed

        FileSystemModel *ptr_model = static_cast<FileSystemModel *>(parent());

	if(ptr_model != nullptr)
	{
                QModelIndex left = ptr_model->index(0, FileSystemModelPublic::SIZE_HEADER);
		QModelIndex right = ptr_model->index(ptr_model->rowCount(),
                        FileSystemModelPublic::SIZE_HEADER);
...
QString FileInfo::getPath() const {return path;}
QString FileInfo::getName() const {return name;}
QString FileInfo::getDate() const {return date;}
QString FileInfo::getFullPath() const {return full_path;}
Qt::CheckState FileInfo::getChecked() const {return checked;}
void FileInfo::setChecked(Qt::CheckState arg_checked) {checked = arg_checked;}
...
bool FileInfo::equals(const FileInfo &rhs) const
{
...
bool FileInfo::contains(const FileInfo &rhs) const
{
	if(type == MFIT_DIR)
	{
...
bool FileInfo::isRedundant(const FileInfo &rhs) const
{
...
bool FileInfo::makesRedundant(const FileInfo &rhs) const
{

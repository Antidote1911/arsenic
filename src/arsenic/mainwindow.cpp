...
MainWindow::MainWindow(QWidget *parent)
  ...
  // General
  connect(m_ui->menuAboutArsenic, &QAction::triggered, this,
          [=] { aboutArsenic(); });
  connect(m_ui->menuHashCalculator, &QAction::triggered, this,
          [=] { hashCalculator(); });
  connect(m_ui->menuAboutQt, &QAction::triggered, this,
          [=] { qApp->aboutQt(); });
  connect(m_ui->menuPassGenerator, &QAction::triggered, this,
          [=] { generator(); });
  connect(m_ui->pushPassGenerator, &QPushButton::clicked, this,
          [=] { generator(); });
  ...
  connect(m_ui->menuConfiguration, &QAction::triggered, this,
          [=] { configuration(); });
  connect(m_ui->menuDarkTheme, &QAction::triggered, this,
          [=](const bool &checked) { dark_theme(checked); });
  connect(m_ui->menuViewButtonToolbar, &QAction::triggered, this,
          [=](const bool &checked) { m_ui->toolBar->setVisible(checked); });
  connect(m_ui->toolBar, &QToolBar::visibilityChanged, this,
          [=](const bool &checked) {
            m_ui->menuViewButtonToolbar->setChecked(checked);
          });
  connect(m_ui->menuArgon2Tests, &QAction::triggered, this,
          [=] { Argon2_tests(); });
  connect(m_ui->tabWidget, &QTabWidget::currentChanged, this,
          [=](const int &index) { switchTab(index); });
  connect(m_ui->comboViewpass, &QCheckBox::stateChanged, this,
          [=](const int &index) { viewPassStateChanged(index); });

  // EncryptPad
  connect(m_ui->menuOpenTxt, &QAction::triggered, this, [=] { openTxtFile(); });
  connect(m_ui->menuSaveTxt, &QAction::triggered, this, [=] { saveTxtFile(); });
  connect(m_ui->menuSaveTxtAs, &QAction::triggered, this,
          [=] { saveTxtFileAs(); });
  connect(m_ui->menuEncryptTxt, &QAction::triggered, this,
          [=] { encryptText(); });
  connect(m_ui->menuDecryptTxt, &QAction::triggered, this,
          [=] { decryptText(); });
  connect(m_ui->menuClearEditor, &QAction::triggered, this,
          [=] { clearEditor(); });
  connect(m_ui->pushEncryptTxt, &QPushButton::clicked, this,
          [=] { encryptText(); });
  connect(m_ui->pushDecryptTxt, &QPushButton::clicked, this,
          [=] { decryptText(); });

  // EncryptFile
  connect(m_ui->menuAddFiles, &QAction::triggered, this, [=] { addFiles(); });
  connect(m_ui->menuRemoveAllFiles, &QAction::triggered, this,
          [=] { clearListFiles(); });
  connect(m_ui->menuEncryptList, &QAction::triggered, this,
          [=] { encryptFiles(); });
  connect(m_ui->menuDecryptList, &QAction::triggered, this,
          [=] { decryptFiles(); });
  connect(m_ui->pushEncrypt, &QPushButton::clicked, this,
          [=] { encryptFiles(); });
  connect(m_ui->pushDecrypt, &QPushButton::clicked, this,
          [=] { decryptFiles(); });
  connect(m_ui->menuAbortJob, &QAction::triggered, this, [=] { abortJob(); });
  connect(Crypto, SIGNAL(updateProgress(QString, qint64)), this,
          SLOT(onPercentProgress(QString, qint64)));
  connect(Crypto, SIGNAL(statusMessage(QString)), this,
          SLOT(onMessageChanged(QString)));
  connect(Crypto, SIGNAL(sucessMessage(QString)), this,
          SLOT(addFinishedFile(QString)));
  connect(Crypto, SIGNAL(sourceDeletedAfterSuccess(QString)), this,
          SLOT(removeDeletedFile(QString)));
...
void MainWindow::addFilePathToModel(const QString &filePath) {
  ...
  if (fileInfo.exists() &&
      fileInfo.isFile()) { // If the file exists, add it to the model
...
void MainWindow::generator() {
  ...
  connect(pwGenerator, SIGNAL(appliedPassword(QString)),
...
void MainWindow::loadPreferences() {
  if (config()->hasAccessError()) {
    QString warn_text = QString(
        tr("Access error for config file %1").arg(config()->getFileName()));
  ...
  m_ui->CheckDeleteFiles->setChecked(
      config()->get("GUI/deleteFinished").toBool());

  switchTab(config()->get("GUI/currentIndexTab").toInt());
  m_ui->tabWidget->setCurrentIndex(
      config()->get("GUI/currentIndexTab").toInt());

...
void HashCheckDialog::calculate(const QString &text)
{
    ...
    if (f_info.isFile() && file.open(QIODevice::ReadOnly)) {
        ...
        if (text == "BLAKE3") {
            ...
            while (!cancel_calculation && (bytes_read = file.read(reinterpret_cast<char *>(buf.data()), block_size)) > 0) {
                blake3_hasher_update(&hasher, buf.data(), bytes_read);
            ...
            if (!cancel_calculation) {
                ...
                m_ui->checksumEdit->setText(QString::fromStdString(Botan::hex_encode(output, BLAKE3_OUT_LEN)));
            }
        } else {
            ...
            if (hash2 && !cancel_calculation) {
                m_ui->checksumEdit->setText(QString::fromStdString(Botan::hex_encode(hash2->final())));
            }

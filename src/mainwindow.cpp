      ...
      m_file_crypto(std::make_unique<Crypto_Thread>(this))
{
    ...
    connect(m_ui->menuChangePassword, &QAction::triggered, this, [=, this] { changePassword(); });
...
void MainWindow::cryptoFileView()
{
    ...
    m_ui->fileListViewCrypto->setAcceptDrops(true);
    m_ui->fileListViewCrypto->viewport()->setAcceptDrops(true);
    m_ui->fileListViewCrypto->viewport()->installEventFilter(this);
}
...
{
    if (obj != m_ui->fileListViewCrypto->viewport())
        ...
        for (const QUrl &url : e->mimeData()->urls()) {
            ...
            if (fi.isDir()) {
                ...
                for (const QFileInfo &child : entries)
                    addFilePathToModel(child.absoluteFilePath());
            } else if (fi.isFile()) {
                addFilePathToModel(path);
            }
        }
        e->acceptProposedAction();
        return true;
...
void MainWindow::changePassword()
{
    ...
    auto *oldPwd  = new QLineEdit;
    auto *newPwd  = new QLineEdit;
    auto *confirm = new QLineEdit;
    oldPwd->setEchoMode(QLineEdit::Password);
    newPwd->setEchoMode(QLineEdit::Password);
    confirm->setEchoMode(QLineEdit::Password);

    layout->addRow(tr("Current password:"), oldPwd);
    layout->addRow(tr("New password:"),     newPwd);
    layout->addRow(tr("Confirm new:"),      confirm);
    ...
    if (oldPwd->text().isEmpty() || newPwd->text().isEmpty()) {
        displayEmptyPassword();
        return;
    }
    ...
    m_file_crypto->setChangePasswordParam(getListFiles(), oldPwd->text(), newPwd->text());
    m_file_crypto->start();
}
